...
#$CSV = "C:\Users\giovani.paganini\powershellson\exchange\BackupDisabledEmails\desabilitados_03022021.csv" #caminho do arquivo csv que possui os UserPrincipalNames dos usuarios
#$arquivo = Import-CSV $CSV -Delimiter ";"

$disabledUsers = Get-ADUser -filter {Enabled -eq $false} -SearchBase "OU=Contas Desabilitadas,OU=Automacao - Autoseg,DC=aviva,DC=com,DC=br" -Properties *
...
function converteCaixaTeste {
    Measure-Command { foreach($user in $disabledUsers){
        $mailbox = (Get-Mailbox -Identity $user.UserPrincipalName -ErrorAction SilentlyContinue)
        ...
        if ($mailbox -ne $null){
            ...
            if(!$isShared){
                Set-Mailbox -Identity $mailbox.UserPrincipalName -Type Shared -WhatIf
                if(!$?){
                    Write-Host "Erro convertendo a caixa de $($user.UserPrincipalName)" -ForegroundColor Red
                } else {
                    Write-Host "Convertido caixa de $($mailbox.UserPrincipalName)" -ForegroundColor Green
                }
            } else {
                Write-Host "Caixa de email $($mailbox.UserPrincipalName) já compartilhada" -ForegroundColor Yellow
            }
        }
    }}
}
